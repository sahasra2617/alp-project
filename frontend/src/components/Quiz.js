          ...
          if (imageSrc) {
            try {
              ...
              if (err.response) {
                ...
                setError(`Error detecting emotion: ${err.response.data.message || 'Unknown error'}`);
              } else if (err.request) {
                console.error('Error request:', err.request);
                setError('Could not connect to emotion detection service. Please try again.');
              } else {
                console.error('Error message:', err.message);
                setError('An unexpected error occurred during emotion detection.');
              }
            }
          }
        }
